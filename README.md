...
### 1.2.1 单机（集中式）架构
### 一个项目包含了所有业务模块，部署到一台服务器上。单机指单个独立的系统实例化部署，集中式指集中的应用（Application）、文件（DATA）、数据（File）
...
- 分布式：一个应用拆分成不同模块部署在多台服务器上。分布式系统是一个硬件或软件组件分布在不同的网络计算机上，彼此之间仅仅通过消息传递进行通信和协调的系统。（服务化架构使搭建分布式系统成为了可能）
- 分布式集群：单纯的分布式和集群不一样，集群挂了一台，其他服务器一样可用，而单纯的分布式只有一个节点的模块系统，一旦节点垮了，这个服务就不可调用，所以就有新的组合玩法分布式集群，分布式架构的某一个模块系统也可以采用集群架构
